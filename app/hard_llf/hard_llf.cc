...
const unsigned int periods[] = {6000, 6000, 4000, 3000, 2000};   // ms for threads A, B, C
const unsigned int wcets[] = {1000, 1000, 1000, 500, 1000};      // ms for threads A, B, C
const unsigned int deadlines[] = {6000, 6000, 4000, 4000, 3500}; // ms for threads A, B, C
...
int main()
{
    ...
    threads[3] = new Periodic_Thread(RTConf(periods[3] * 1000, deadlines[3] * 1000, wcets[3] * 1000, 0, iterations), &func_d);
    threads[4] = new Periodic_Thread(RTConf(periods[4] * 1000, deadlines[4] * 1000, wcets[4] * 1000, 0, iterations), &func_e);
    threads[5] = new Periodic_Thread(RTConf(period_w * 1000, deadline_w * 1000, wcet_w * 1000, 0, 200), &func_w);
...
int func_d()
{
    ...
    do
    {
        cout << "D running - Priority: " << threads[3]->priority() << endl;
        matrix_multiply(0, 1, 2, 3, 4);
        cout << "D Finished - Priority: " << threads[3]->priority() << endl;
...
int func_e()
{
    ...
    do
    {
        cout << "E running - Priority: " << threads[4]->priority() << endl;
        matrix_multiply(0, 1, 2, 3, 4);
        cout << "E Finished - Priority: " << threads[4]->priority() << endl;
...
void exec(char c, unsigned int time)
{
    ...
    cout << elapsed << "\t" << c << "\t Priority levels: [A=" << threads[0]->priority() << ", B=" << threads[1]->priority() << ", C=" << threads[2]->priority() << ", D=" << threads[3]->priority() << ", E=" << threads[4]->priority() << "]" << endl;
