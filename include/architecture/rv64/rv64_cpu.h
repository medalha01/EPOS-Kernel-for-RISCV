...
private:
    ...
    static const bool multicore = (Traits<Machine>::CPUS > 1);
    static const int number_of_cores = (Traits<Machine>::CPUS);
...
public:
    ...
    static T finc(volatile T &value)
    {
        T old;
        if (sizeof(T) == sizeof(Reg64))
            ASM("amoadd.d %0, %2, (%1)" : "=r"(old) : "r"(&value), "r"(1) : "cc", "memory");
        else
            ASM("amoadd.w %0, %2, (%1)" : "=r"(old) : "r"(&value), "r"(1) : "cc", "memory");
        return old;
    }
...
public:
    ...
    {
        Reg r;
        ASM("csrr %0, mhartid" : "=r"(r) : : "memory", "cc");
        return r;
    }
    ...
    static void mtvec(Reg m, Phy_Addr a)
    {
        Reg p = (a & -4UL) | (m & 0x3);
        ASM("csrw mtvec, %0" : : "r"(p) : "cc");
    }
    static Reg mtvec()
    {
        Reg r;
        ASM("csrr %0, mtvec" : "=r"(r) : :);
        return r;
    }
    ...
    {
        Reg r;
        ASM("csrr %0, mcause" : "=r"(r) : :);
        return r;
    }
    static Reg mtval()
    {
        Reg r;
        ASM("csrr %0, mtval" : "=r"(r) : :);
        return r;
    }
    ...
    static void stvec(Reg m, Phy_Addr a)
    {
        Reg p = (a & -4UL) | (m & 0x3);
        ASM("csrw stvec, %0" : : "r"(p) : "cc");
    }
    static Reg stvec()
    {
        Reg r;
        ASM("csrr %0, stvec" : "=r"(r) : :);
        return r;
    }
    ...
    {
        Reg r;
        ASM("csrr %0, scause" : "=r"(r) : :);
        return r;
    }
    static Reg stval()
    {
        Reg r;
        ASM("csrr %0, stval" : "=r"(r) : :);
        return r;
    }
    ...
    static void satp(Reg r)
    {
        ASM("csrw satp, %0" : : "r"(r) : "cc");
