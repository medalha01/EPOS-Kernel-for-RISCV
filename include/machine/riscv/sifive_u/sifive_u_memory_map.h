...
public:
    enum : unsigned long
    {
        ...
        INT_M2S = LAST_PAGE,
        LOCK_BASE = INT_M2S - 4096,                                                      // the last page is used by the _int_m2s() machine mode interrupt forwarder installed by SETUP before going into supervisor mode; code and stack share the same page, with code at the bottom and the stack at the top
        FLAT_MEM_MAP = LOCK_BASE - 512,                                                  // in LIBRARY mode, a single-level mapping of the whole memory space is used
        BOOT_STACK = FLAT_MEM_MAP - Traits<Machine>::STACK_SIZE * Traits<Machine>::CPUS, // will be used as the stack's base, not the stack pointer
        FREE_BASE = RAM_BASE,
