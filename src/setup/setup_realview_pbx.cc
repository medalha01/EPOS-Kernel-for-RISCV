...
void Setup::setup_flat_paging()
{
    ...
    for(CPU::Reg i = MMU_Common<10,10,12>::pdi(RAM_BASE); i < MMU_Common<10,10,12>::pdi(RAM_BASE) + MMU_Common<10,10,12>::pages(RAM_TOP - RAM_BASE); i++)
        pt[i] = (i << 20) | ARMv7_MMU::Section_Flags::FLAT_MEMORY_MEM;
    for(CPU::Reg i = MMU_Common<10,10,12>::pdi(MIO_BASE); i < MMU_Common<10,10,12>::pdi(MIO_BASE) + MMU_Common<10,10,12>::pages(MIO_TOP - MIO_BASE) - 1; i++)
