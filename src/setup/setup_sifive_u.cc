...
class Setup {
	private:
		...
		// architecture.h will use No_MMU if multitasking is disable, but we
		// need the correct MMU for the Flat Memory Model.
		typedef IF<Traits<CPU>::WORD_SIZE == 32, SV32_MMU, SV39_MMU>::Result MMU; 
...
void Setup::setup_flat_paging()
{
	...
	static const unsigned long PD_ENTRIES = (Math::max(RAM_TOP, MIO_TOP)
			- Math::min(RAM_BASE, MIO_BASE)
...
void Setup::say_hi()
{
	...
	//	<< " MHz (BUS clock = " << Traits<Machine>::HFCLK / 1000000
	//	<< " MHz)" << endl;
	...
	//	kout << "  Extras:       " << si->lm.app_extra_size << " bytes"
	//		<< endl;
...
{
	// architecture.h will use No_MMU if multitasking is disable, 
	// but we need the correct MMU for the Flat Memory Model.
	typedef IF<Traits<CPU>::WORD_SIZE == 32, SV32_MMU, SV39_MMU>::Result MMU;

	if(CPU::mhartid() == 0) {                             
		// SiFive-U has 2 cores, but core 0 (an E51) does not feature an MMU,
		// so we halt it and let core 1 (an U54) run in a single-core configuration
		CPU::halt();
	}
	...
	if (Traits<Machine>::supervisor) {
		...
		// prepare jump into supervisor mode at MRET
		// with interrupts enabled at machine level
...
// continue in machine mode at MRET
	...
	// enter supervisor mode at setup (mepc)
	// with interrupts enabled (mstatus.mpie = true)
	CPU::mret();                                      
}
...
// RISC-V's CLINT triggers interrupt 7 (MTI) whenever MTIME == MTIMECMP and
// there is no way to instruct it to trigger interrupt 9 (STI). So, even if we
// delegate all interrupts with MIDELEG, MTI doesn't turn into STI and MTI is
// visible in SIP. In other words, MTI must always be handled in machine mode,
// although the OS will run in supervisor mode. Therefore, an interrupt
// forwarder must be installed in machine mode to catch MTI and manually trigger
// STI. We use RAM_TOP for this, with the code at the beginning of the last page
// and per-core 256 bytes stacks at the end of the same page.
void _int_m2s() {
	// Save context
	...
	if ((id & CLINT::INT_MASK) == CLINT::IRQ_MAC_TIMER) {
		Timer::reset(); // MIP.MTI is a direct logic on (MTIME == MTIMECMP) and
						// reseting the Timer (i.e. adjusting MTIMECMP) seems to be
